...
func SeedDb(SQLDB *db.SQLDatabase) {
	// Seed the database with temp data to meet the scenario requirements

	SQLDB.Stations = append(SQLDB.Stations, stations...)
	SQLDB.Routes = append(SQLDB.Routes, routes...)
	SQLDB.Services = append(SQLDB.Services, services...)
	SQLDB.Carriages = append(SQLDB.Carriages, carriages...)
